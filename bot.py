...
async def send_to(bot, chat_id):
    ...
    if total == 0:
        await bot.send_message(chat_id=chat_id, text="Novostey ne naydeno.")
        return
    msg = "Dajdest finansirovaniya - " + str(total) + " novostey\n\n"
...
async def start(update, context):
    ...
    s.add(cid)
    save_subs(s)
    await update.message.reply_text("Podpisan! /news - poluchit seychas.")
...
async def stop(update, context):
    ...
    s.discard(cid)
    save_subs(s)
    await update.message.reply_text("Otpisan.")

async def news_now(update, context):
    await update.message.reply_text("Sobirayu...")
